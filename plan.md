...
- 点击「夜深了...」后，前端新建观赛对局的 `start_paused=true -> 建立 SSE -> POST /run` 握手期间必须显示 UI-only 启动提示：`后端服务正在启动，请耐心等待`。该提示可展示已等待秒数和加载动效，但不得写入 EventLog、narrative、SSE、manifest 或 replay/resimulate 校验，也不得改变 ack、pacing、SSE cursor 或后端启动接口语义；收到非 `GAME_START` 的正式阶段后立即清除。
