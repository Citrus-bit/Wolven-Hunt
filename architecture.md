...
- `WH_REVIEW_PROVIDER`：赛后 AI 复盘报告 provider，`mock | litellm`，默认 `litellm`；`litellm` 在线成功时生成 `generation_mode=real_ai`，`mock` 生成 `generation_mode=offline_mock`
